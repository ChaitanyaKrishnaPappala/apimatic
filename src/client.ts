...
function getBaseUri(server: Server = 'default', config: Configuration): string {
  if (config.environment === Environment.Production) {
    if (server === 'default') {
      return 'https://datalabs-api.tapestry.com';
    }
  }
  if (config.environment === Environment.Develop) {
    if (server === 'default') {
      return 'https://datalabs-dev-api.tapestry.com';
    }
  }
  if (config.environment === Environment.Staging) {
    if (server === 'default') {
      return 'https://datalabs-staging-api.tapestry.com';
    }
  }
  if (config.environment === Environment.Test) {
    if (server === 'default') {
      return 'https://api-test.tapestrydev.com';
    }
  }
...
function withXApiKey({ xApiKey }: { xApiKey: string }) {
  return (rb: SdkRequestBuilder) => {
    rb.interceptRequest(request => {
      ...
      setHeader(headers, 'x-api-key', xApiKey);
